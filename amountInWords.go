...
func ConvertAmountInIndianWords(amount string) (output string, err error) {
	...
	if len(nums) >= 1 {
		...
		if whole == "" && fraction != "" {
			// finalOutPut = 'paise ' + fraction + ' only';
			finalOutPut = fraction + "paise only."
		}
		if whole != "" && fraction == "" {
			// finalOutPut = "Rupees " + whole + " only";
			finalOutPut = whole + "rupees only."
		}
		if whole != "" && fraction != "" {
			// finalOutPut = "Rupees " + whole + "and paise " + fraction + " only";
...
func main() {
	...
	amt, err := ConvertAmountInIndianWords(s)
	if err != nil {
		fmt.Println(err)
		return
	}

	fmt.Println(strings.ToUpper(amt))
